...
class BackboneCNN(keras.Model):
    def __init__(self):
        ...
        self.layer1 = Conv2D(filters=64, kernel_size=[11, 11], strides=[1, 1], activation='relu', name="Conv1")
        self.pool1 = MaxPooling2D(pool_size=[3, 3], strides=[2, 2])
        # self.dropout1 = Dropout(rate=0.5)
        self.batchnorm1 = BatchNormalization()
        self.layer2 = Conv2D(filters=128, kernel_size=[7, 7], strides=[1, 1], activation='relu', name="Conv2")
        self.pool2 = MaxPooling2D(pool_size=[3, 3], strides=[2, 2])
        # self.dropout2 = Dropout(rate=0.5)
        self.batchnorm2 = BatchNormalization()
        self.layer3 = Conv2D(filters=128, kernel_size=[7, 7], strides=[1, 1], activation='relu', name="Conv3")
        self.pool3 = MaxPooling2D(pool_size=[3, 3], strides=[2, 2])
        # self.dropout3 = Dropout(rate=0.5)
        self.batchnorm3 = BatchNormalization()
        self.layer4 = Conv2D(filters=256, kernel_size=[3, 3], strides=[1, 1], activation='relu', name="Conv4")
        self.pool4 = MaxPooling2D(pool_size=[3, 3], strides=[2, 2])
        # self.dropout4 = Dropout(rate=0.5)
        self.batchnorm4 = BatchNormalization()
        self.layer5 = Conv2D(filters=256, kernel_size=[1, 1], strides=[1, 1], activation='relu', name="Conv5")
